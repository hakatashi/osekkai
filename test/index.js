...
describe('osekkai', () => {
	...
	describe('Core', () => {
		it('should accept array of chunks as first argument', () => {
			expect(osekkai.bind(osekkai, ['暇を持て余した', '神々の', '遊び'])).not.toThrow();
		});

		it('should format array of chunks as array', () => {
			expect(osekkai(['暇を持て余した', '神々の', '遊び']).format('plain')).toEqual(['暇を持て余した', '神々の', '遊び']);
		});
		...
		it('should error when unknown formatter is specified', () => {
			const obj = osekkai('ちくわ大明神');
			expect(obj.format.bind(obj, 'UNKNOWN')).toThrow();
		});
